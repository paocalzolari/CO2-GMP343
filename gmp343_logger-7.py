...
def get_filenames(config):
    """Genera i nomi file raw e _min per il giorno corrente."""
    today     = datetime.utcnow().strftime("%Y%m%d")
    basename  = config.get("output", "basename",  fallback="carbocap")
    extension = config.get("output", "extension", fallback="raw")
    site_name = config.get("location", "name",    fallback="unknown")
    data_dir  = get_data_dir(config)
    raw_file  = os.path.join(data_dir, f"{basename}-{site_name}-{today}.{extension}")
    avg_file  = os.path.join(data_dir, f"{basename}-{site_name}-{today}_min.{extension}")
    return raw_file, avg_file

def write_headers_if_needed(raw_file, avg_file, config):
    """
    Writes headers to the files if they do not already exist.
    RAW: "YYYY MM DD hh mm ss CO2 ppm"
    AVG: "YYYY MM DD HH(UTC) mm ss CO2(ppm) std-CO2 n-samples"
    """
    raw_header = "YYYY MM DD hh mm ss CO2 ppm"
    avg_header = "YYYY MM DD HH(UTC) mm ss CO2(ppm) std-CO2 n-samples"
        ...
        try:
            ...
            if line:
                ...
                if value is not None:
                    ...
                    if current_timestamp.replace(second=0, microsecond=0) == current_minute:
                        ...
                        with open(avg_file, 'a') as f_avg:
                            ...
                            f_avg.write(f"{ts_avg} {avg:.2f} {std:.2f} {n}\n")
                ...
                if now.replace(second=0, microsecond=0) != current_minute:
                    with open(avg_file, 'a') as f_avg:
                        ts_avg = current_minute.strftime("%Y %m %d %H %M %S")
                        f_avg.write(f"{ts_avg} 999.99 0.00 0\n")
