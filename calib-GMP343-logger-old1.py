...
Formato file v2 (dal 2026):
  - Nome: carbocap343-<site>-<YYYYMMDD>_p00.raw e _p00_min.raw
  - Data/ora: YYYY-MM-DD HH:MM:SS (con trattini e due punti)
  - Header _min: #date time CO2[PPM] CO2_std[%] ndata_60s_mean flag
  - Std in percentuale: std_pct = (std / mean) * 100
...
def get_filenames(cfg) -> tuple:
    """Restituisce (raw_file, min_file) per oggi con formato v2."""
    ...
    raw_file  = os.path.join(data_dir, f"{basename}-{site}-{today}_p00.{extension}")
    min_file  = os.path.join(data_dir, f"{basename}-{site}-{today}_p00_min.{extension}")
...
def write_headers_if_needed(raw_file: str, min_file: str):
    ...
    if not os.path.exists(min_file):
        with open(min_file, "w") as f:
            f.write("#date time CO2[PPM] CO2_std[%] ndata_60s_mean flag\n")
            ...
            try:
                ...
                if now_min == cur_min:
                    ...
                    if co2_buf:
                        ...
                        avg, std_pct, n = 999.99, 0.0, 0
                    with open(min_file, "a") as f:
                        f.write(f"{ts_avg} {avg:.2f} {std_pct:.2f} {n} {min_flag}\n")
                    cur_min  = now_min
                    co2_buf  = [value]
                    flag_buf = [self.flag]
