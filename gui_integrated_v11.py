...
# -*- coding: utf-8 -*-
"""
GMP343 Monitor Integrato v11
Tab 1 : Monitor real-time
Tab 2 : Grafico CO₂

Percorsi:
  File ini  : ~/programs/CO2/config/   (fisso, sempre)
  Eseguibile: /opt/co2-monitor/        (quando compilato)
  Immagine  : accanto all'eseguibile
  Dati      : definito in name.ini → data_path (default ~/data)
"""
...
def read_file(path: str):
    """Legge un file dati; restituisce liste (times, values, stds, counts)."""
    times, values, stds, counts = [], [], [], []
    if not path or not os.path.exists(path):
        return times, values, stds, counts
    try:
...
def load_period(cfg: configparser.ConfigParser,
    ...
    all_t, all_v, all_s, all_c = [], [], [], []
    for i in range(n_days):
        d = start + timedelta(days=i)
        t, v, s, c = read_file(build_filename(cfg, d))
        all_t.extend(t)
        all_v.extend(v)
        all_s.extend(s)
        all_c.extend(c)
...
class GraphWidget(QWidget):
    ...
    def _init_axes(self):
        ...
        # Linea dati + punto hover
        self.line, = self.ax.plot(
            [], [], "o-",
            markersize=3.5, linewidth=1.3,
            color="#2060c0", picker=6
        )
        ...
        # Tooltip - usa axes fraction per posizionamento assoluto sicuro
        self.annot = self.ax.annotate(
            "", xy=(0, 0),
            xycoords="data",
            xytext=(0.02, 0.95),          # default: angolo alto-sinistra
        ...
        # Traccia zoom manuale tramite cambio limiti
    ...
    def _reload(self):
        ...
        if result is None:
            self.line.set_data([], [])
            self.ax.set_title("Nessun dato disponibile", fontsize=11)
            self._set_x_axis(start, n_days)
            self.canvas.draw_idle()
            self._ignore_lim_change = False
            return

        times, values, stds, counts = result
...
class GMP343Monitor(QMainWindow):
    ...
    def _build_monitor_tab(self):
        ...
        self.lbl_file.setWordWrap(True)       # path lungo va a capo
